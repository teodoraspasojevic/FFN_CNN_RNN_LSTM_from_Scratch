...
class BatchNormalization(BaseLayer):
    ...
    def reformat(self, input_tensor):
        if input_tensor.ndim == 4:
            ...
            reformated_tensor = np.reshape(input_tensor, newshape=(b, h, m*n))
            reformated_tensor = reformated_tensor.transpose(0, 2, 1)
            output_tensor = reformated_tensor.reshape(b*m*n, h)
            return output_tensor
        elif input_tensor.ndim == 2:
            b, h, m, n = self.original_shape
            reformated_tensor = input_tensor.reshape(b, m*n, h)
            reformated_tensor = reformated_tensor.transpose(0, 2, 1)
            output_tensor = reformated_tensor.reshape(b, h, m, n)
            return output_tensor
        else:
            return input_tensor
